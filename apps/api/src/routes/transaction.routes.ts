...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    res.status(201).json({
      ...
      description: transaction.metadata?.description || description,
      reference_id: transaction.referenceId,
      idempotency_key: transaction.idempotencyKey,
      created_by: transaction.metadata?.createdBy || `api_key:${req.tenantId}`,
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    res.status(201).json({
      debit_transaction: {
        ...
        description: result.debitTransaction.metadata?.description || description,
        reference_id: result.debitTransaction.referenceId,
        idempotency_key: result.debitTransaction.idempotencyKey,
        created_by: result.debitTransaction.metadata?.createdBy || `api_key:${req.tenantId}`,
        is_sandbox: req.isSandbox || false,
      ...
      credit_transaction: {
        ...
        description: result.creditTransaction.metadata?.description || description,
        reference_id: result.creditTransaction.referenceId,
        idempotency_key: result.creditTransaction.idempotencyKey,
        created_by: result.creditTransaction.metadata?.createdBy || `api_key:${req.tenantId}`,
        is_sandbox: req.isSandbox || false,
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    res.status(201).json({
      transaction_id: transaction.id,
      type: transaction.type,
      original_tx_id: transaction.metadata?.originalTxId,
      amount: transaction.amount.toFixed(4),
      balance_before: transaction.balanceBefore.toFixed(4),
      balance_after: transaction.balanceAfter.toFixed(4),
      description: `Reversal of: ${transaction.metadata?.originalDescription}`,
      created_at: transaction.createdAt,
    });
  }
);
