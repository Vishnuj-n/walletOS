...
describe('Reversal Tests', () => {
  ...
  let latestTenantId: string | null = null;
  ...
  afterEach(async () => {
    if (latestTenantId) {
      await cleanupTestData(latestTenantId);
      latestTenantId = null;
    }
  });
  ...
  describe('POST /api/v1/transactions/:txId/reverse', () => {
    it('should reverse a credit transaction successfully', async () => {
      ...
      expect(creditResponse.status).toBeGreaterThanOrEqual(200);
      expect(creditResponse.status).toBeLessThan(300);
    ...
    it('should reverse a debit transaction successfully', async () => {
      ...
      const setupCreditResponse = await request(app)
        .post('/api/v1/transactions/credit')
        .set('x-api-key', apiKey.plainKey)
      ...
      expect(setupCreditResponse.status).toBeGreaterThanOrEqual(200);
      expect(setupCreditResponse.status).toBeLessThan(300);
      ...
      expect(debitResponse.status).toBeGreaterThanOrEqual(200);
      expect(debitResponse.status).toBeLessThan(300);
